...
export const DOCUMENTO_PREVIEW_MODELS = [{ id: 'florescer', label: 'Modelo padrão' }]
...
export const DOCUMENTO_TEMPLATES: DocumentoTemplate[] = [
  {
    id: 'blank',
    label: 'Em branco',
    category: 'Documento',
...
export function loadCustomDocumentoTemplates(): DocumentoTemplate[] {
...
export function saveCustomDocumentoTemplate({
  label,
  title,
  content,
...
export function getAllDocumentoTemplates(): DocumentoTemplate[] {
  return [...DOCUMENTO_TEMPLATES, ...loadCustomDocumentoTemplates()]
}

export function findDocumentoTemplate(id: string): DocumentoTemplate {
...
export function documentoPreviewText(item: { content?: string | null }): string {
  const value = htmlToPlainText(item?.content || '')
  if (!value) return 'Sem conteúdo ainda.'
  return value.length > 140 ? `${value.slice(0, 140)}…` : value
}

export function documentoStatusLabel(status?: string | null): string {
