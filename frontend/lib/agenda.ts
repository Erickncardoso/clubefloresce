...
export function layoutAgendaEvent(
  ...
  const height = Math.max(
    ((Number(item.durationMin) || 60) / 60) * hourHeightPx - 3,
    24,
  );
  return { top, height };
}
...
export async function fetchAgendaAppointments(
  ...
  return apiFetch<{ appointments: AgendaAppointment[] }>(
    `/agenda/appointments${suffix}`,
  );
}
...
export async function updateAgendaAppointment(
  id: string,
  payload: Partial<AgendaAppointmentInput>,
) {
  return apiFetch<{ appointment: AgendaAppointment }>(
