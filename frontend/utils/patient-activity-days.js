import {
  ...
  weekStartKey,
  parseFoodDays,
} from './patientGoalsProgress.js'

export function goalsAveragePercent(goals, progress, date = new Date()) {
  const summary = buildGoalsSummary(goals, progress, date)
...
export function isActivityDay(goals, progress, dateKeyStr, diaryEntryCount = 0) {
  ...
  for (const goal of goals) {
    ...
    if (getProgress(goal, progress, date) > 0) return true
  }
...
export function buildMonthActivityMap(goals, progress, diaryDays = []) {
  ...
  for (const [key] of Object.entries(progress)) {
...
export function countActiveDaysInMonth(activityMap, year, month) {
  let count = 0
...
export function countActiveDaysInWeek(activityMap, date = new Date()) {
  return getWeekDateKeys(date).filter((key) => activityMap.get(key)).length
}

export function computeCurrentStreak(activityMap, todayKey = dateKey()) {
  let streak = 0
...
export function computeBestStreak(activityMap) {
