...
import { CHATS_POLL_INTERVAL_MS, getWhatsappApiBase, getProxyBase, whatsappFetchInit, whatsappJsonHeaders } from './useWhatsappApi.js'
...
/** Consulta cache/API para detectar conversas já sincronizadas (ex.: página Conexão). */
export async function probeInitialSyncProgress() {
  ...
  try {
    ...
    const findRes = await fetch(`${proxyBase}/chat/find`, {
      method: 'POST',
    ...
    const liveCount = Array.isArray(findData?.chats) ? findData.chats.length : 0
    if (liveCount > 0) noteInitialSyncChatCount(liveCount)
  } catch {
    /* silencioso */
  } finally {
    probeInFlight = false
