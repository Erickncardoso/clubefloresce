/**
 * Gera ícone 1024×1024, splash e adaptive icon a partir do logo SVG.
...
function logoSvg(fill, width, height) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 295 415">
    <path d="${LOGO_PATH}" fill="${fill}"/>
  </svg>`;
}

async function renderLogoOnCanvas(size, options = {}) {
  const {
    background = BRAND,
    logoFill = '#FFFFFF',
    logoScale = 0.62,
  ...
  const logoHeight = Math.round(size * logoScale);
  const logoWidth = Math.round((logoHeight * 295) / 415);
  const logoBuffer = await sharp(Buffer.from(logoSvg(logoFill, logoWidth, logoHeight)))
    .png()
    .toBuffer();
...
async function renderSplash() {
  ...
  const logoWidth = Math.round((logoHeight * 295) / 415);
  const logoBuffer = await sharp(Buffer.from(logoSvg(BRAND, logoWidth, logoHeight)))
    .png()
    .toBuffer();
...
const icon = await renderLogoOnCanvas(1024);
await writeFile(path.join(assetsDir, 'icon.png'), icon);

const splashLogo = await renderLogoOnCanvas(512, {
  background: SPLASH_BG,
  logoFill: BRAND,
  logoScale: 0.72,
});
...
console.log('Ícones gerados em expo-app/assets/');
