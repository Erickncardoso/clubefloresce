...
function notesStorageKey(lessonId: string) {
...
export async function loadLessonNotes(lessonId: string): Promise<LessonNote[]> {
  if (!lessonId) return [];
  const raw = await AsyncStorage.getItem(notesStorageKey(lessonId));
  if (!raw) return [];
  try {
...
export async function saveLessonNotes(lessonId: string, notes: LessonNote[]) {
  if (!lessonId) return;
  await AsyncStorage.setItem(notesStorageKey(lessonId), JSON.stringify(notes));
}
