/**
 * Integração legada UAZAPI (desativada quando WHATSAPP_PROVIDER=wuzapi).
 * Mantida intacta para referência e rollback — não remover até WuzAPI estar completa.
 */
...
/** Cache curto da lista completa GET /instance/all — reduz hot-path em status/proxy. */
let allInstancesCache: { data: any[]; expiresAt: number } | null = null;
const ALL_INSTANCES_TTL_MS = 20_000; // 20s
...
export class UazapiWhatsappService {
  ...
  private async _fetchAllInstances(): Promise<any[]> {
    if (allInstancesCache && allInstancesCache.expiresAt > Date.now()) {
      return allInstancesCache.data;
    }
    try {
      ...
      if (res.ok) {
        ...
        allInstancesCache = { data: instances, expiresAt: Date.now() + ALL_INSTANCES_TTL_MS };
        return instances;
      }
  ...
  /** Invalida o cache de GET /instance/all (create/delete/cleanup/connect). */
  invalidateAllInstancesCache(): void {
  ...
  async cleanupDuplicateInstancesForUser(userId: string, keepCanonical = true): Promise<number> {
    ...
    if (deleted > 0) {
      this.invalidateAllInstancesCache();
  ...
  async getStatus(userId: string): Promise<any> {
    try {
      // Cleanup de duplicatas fica em connect/create (_ensureInstanceForUser), não no poll de status.
      const instances = await this._fetchAllInstances();
  ...
  async purgeLocalSessionData(userId: string): Promise<void> {
    await this.purgeWhatsappDatastore(userId);
    instanceTokenCache.delete(userId);
    instanceProfilePicCache.delete(userId);
